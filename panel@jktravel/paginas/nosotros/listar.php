...
include("../../conexion/conexion.php");
	...
	{
		$rst_query=mysql_query("SELECT * FROM ub_inicio WHERE id>0 ORDER BY titulo ASC;", $conexion);
		...
		$rst_query=mysql_query("SELECT * FROM ub_inicio WHERE id>0 ORDER BY titulo ASC LIMIT $inicio, $registros;", $conexion);
	...
	{
		$rst_query=mysql_query("SELECT * FROM ub_inicio WHERE titulo LIKE '%$buscar%' ORDER BY titulo ASC;", $conexion);
		$num_registros=mysql_num_rows($rst_query);
		...
		$rst_query=mysql_query("SELECT * FROM ub_inicio WHERE titulo LIKE '%$buscar%' ORDER BY titulo ASC LIMIT $inicio, $registros;", $conexion);
		$paginas=ceil($num_registros/$registros);
...
<link rel="stylesheet" type="text/css" href="../../css/style-listas.css">
                      ...
                      <td><table width="95%" align="center" cellpadding="5" cellspacing="0" id="cebreado-php">
                        <thead>
                          <tr class="titulo-campo">
                            ...
                            <th width="10%" align="center">IDIOMA</th>
                            <?php 
  ...
  ?>
                          <tr<?php echo alt($zebra); $zebra++; ?>>
                            <td width="50%"><p><?php echo $fila["titulo"]; ?></p></td>
                            <td width="10%" align="center"><?php 
						if($fila["idioma"]=="1")
						{
							echo "Espa&ntilde;ol";
						}elseif ($fila["idioma"]=="2")
						{	
							echo "Catal&aacute;n";
						}
					?></td>
                    		<?php 
								if($fila_query3["modificar"]==1){
                            		echo "<td width='10%' align='center'><a href='form-modificar.php?id=<?php echo $fila['id'] ?>' target='mainFrame'><strong>Modificar</strong></a></td>";
								}
							?>
                            
                            <?php 
								if($fila_query3["eliminar"]==1){
                            		echo "<td width='10%' align='center'><a href='form-eliminar.php?id=<?php echo $fila['id'] ?>'><strong>Eliminar</strong></a></td>";
								}
							?>
