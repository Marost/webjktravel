...
$cebra=1;
	...
	{
		$rst_query=mysql_query("SELECT * FROM jk_paquetes WHERE id>0 ORDER BY tipo_paquete ASC;", $conexion);
		...
		$rst_query=mysql_query("SELECT * FROM jk_paquetes WHERE id>0 ORDER BY tipo_paquete ASC LIMIT $inicio, $registros;", $conexion);
	...
	{
		$rst_query=mysql_query("SELECT * FROM jk_paquetes WHERE destino LIKE '%$buscar%' ORDER BY tipo_paquete ASC;", $conexion);
		$num_registros=mysql_num_rows($rst_query);
		...
		$rst_query=mysql_query("SELECT * FROM jk_paquetes WHERE destino LIKE '%$buscar%' ORDER BY tipo_paquete ASC LIMIT $inicio, $registros;", $conexion);
		$paginas=ceil($num_registros/$registros);
	...
	//-------- CARGAR TIPO DE PAQUETES
	$rst_query2=mysql_query("SELECT * FROM jk_tipo_paquete",$conexion);
...
<link rel="stylesheet" type="text/css" href="../../css/style-listas.css">
                      ...
                      <table width="95%" align="center" cellpadding="5" cellspacing="0" id="cebreado-php">
                        <thead>
                          <tr class="titulo-campo">
                            <th width="50%" height="25">DESTINO ||-|| DIAS</th>
                            <th width="30%" align="center">TIPO PAQUETE</th>
                            <?php 
  ...
  ?>
                          <tr<?php echo alt($zebra); $zebra++; ?>>
                            <td width="50%"><p><strong class="up"><?php echo $fila["destino"]; ?></strong> ||-|| <?php echo $fila["paquete"]; ?></p></td>
                            <td width="30%" align="center"><p><?php echo $fila["nombre_tp"]; ?></p></td>
                            
                            <?php 
								if($fila_query3["modificar"]==1){
                            		echo "<td width='10%' align='center'><a href='form-modificar.php?id=";
									echo $fila['id'];
                            ...
                            <?php 
								if($fila_query3["eliminar"]==1){
                            		echo "<td width='10%' align='center'><a href='form-eliminar.php?id=";
									echo $fila['id'];
